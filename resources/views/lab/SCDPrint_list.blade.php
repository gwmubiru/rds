...
?>
<?php 
	$print_url="/printSCD?printed=no";
	$printed_url="/printSCD?printed=yes";
	...
	$selected_batches = DispatchManager::getBatchesForSCDPrint();
...
<div id="content">
			...
			<tr class="{{$tr_css}}"  role="alert">
				...
				<td align="center">
					...
					<div style="float:left; {{ $print_button_visibility}}" >
						<a href="/sc_result_slips?b={{$w->id}}&scd=y" target="_blank" class="btn {{ $btn_class }}"><i  class="glyphicon {{ $icon_class }}"></i>&nbsp;Batch</a>
					</div>	
	...
	<p style="text-align: center">
		...
		<a href="/printSCD?pg={{ $prev_pg }}{{ $url_params }}" class="btn {{ $class_prev_link }}">&laquo; Newer Batches</a>

		@if(Request::has('e404'))
			<a href="/printSCD" id="reset_display" class="btn">[ Go To : Today's Batches ]</a>
		@else
			<a href="/printSCD?{{ $url_params }}" id="reset_display" class="btn {{$class_prev_link}}">[ This is : Page {{ $pg }} ]</a>
		@endif

		<a href="/printSCD?pg={{ $next_pg }}{{ $url_params }}" class="btn ">Older Batches &raquo; </a>
		...
		$('#tab_id').dataTable({
    		...
    		"fnDrawCallback": function( oSettings ) {
        		...
        		if ($(".dataTables_empty")[0]) {
					...
					deep_search.attr("href", "/printSCD?e404=" + batch_to_find)
